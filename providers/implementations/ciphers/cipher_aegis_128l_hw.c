...
static ossl_inline aes_block_t AES_BLOCK_XOR(const aes_block_t a, const aes_block_t b)
{
    const aes_block_t out = { a.w0 ^ b.w0, a.w1 ^ b.w1, a.w2 ^ b.w2, a.w3 ^ b.w3 };
    return out;
}

static ossl_inline aes_block_t AES_BLOCK_AND(const aes_block_t a, const aes_block_t b)
{
    const aes_block_t out = { a.w0 & b.w0, a.w1 & b.w1, a.w2 & b.w2, a.w3 & b.w3 };
    return out;
}

static ossl_inline aes_block_t AES_BLOCK_LOAD(const uint8_t in[16])
{
    const aes_block_t out = { LOAD32_LE(in + 0), LOAD32_LE(in + 4), LOAD32_LE(in + 8),
                              LOAD32_LE(in + 12) };
    return out;
}

static ossl_inline aes_block_t AES_BLOCK_LOAD_64x2(const uint64_t a, const uint64_t b)
{
    const aes_block_t out = { (uint32_t) b, (uint32_t) (b >> 32), (uint32_t) a,
                              (uint32_t) (a >> 32) };
    return out;
}

static ossl_inline void AES_BLOCK_STORE(uint8_t out[16], const aes_block_t in)
{
...
static const uint32_t aes_lut[1024] = {
...
static const uint32_t *const LUT0 = aes_lut + 0 * 256;
static const uint32_t *const LUT1 = aes_lut + 1 * 256;
static const uint32_t *const LUT2 = aes_lut + 2 * 256;
static const uint32_t *const LUT3 = aes_lut + 3 * 256;

static ossl_inline aes_block_t AES_ENC(const aes_block_t block, const aes_block_t rk)
{
    aes_block_t    out;
...
static ossl_inline void aegis_128l_absorb(const uint8_t * const src,
                                 ...
                                 size_t ad_len)
{
    ...
    if (left != 0) {
        ...
        aegis_128l_absorb(ctx->buf, state);
        ad += RATE - left;
        ad_len -= RATE - left;
    }
    for (i = 0; i + RATE * 2 <= ad_len; i += RATE * 2) {
        ...
        aegis_128l_update(state, msg0, msg1);
        aegis_128l_update(state, msg2, msg3);
    }
    for (; i + RATE <= ad_len; i += RATE) {
        aegis_128l_absorb(ad + i, state);
    }
    if (i < ad_len) {
...
static void aegis_128l_mac(uint8_t *mac, size_t maclen, uint64_t adlen,
                           uint64_t mlen, aes_block_t * const state)
{
    aes_block_t tmp;
    int i;
