...
typedef softaes_block_t aes_block_t;
    ...
    size_t inl)
{
    ...
    /* Absorb a partial AAD block once, before the payload or final tag. */
    if (ctx->msg_len == 0 && (out != NULL || in == NULL)) {
        size_t left = ctx->ad_len % RATE;
